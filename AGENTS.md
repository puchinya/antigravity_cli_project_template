...
3. **実装完了の定義 (Definition of Done: DoD) と仕様書の自動同期ルール (AI必須チェックゲート):**
   ...
   - コミットおよびプルリクエストの作成時には、これらドキュメントの同期更新も同一コミット（またはPR）に必ず含めてください。
...
1. **コンテナ内開発の原則 (Containerized Development):**
   - ソースコードのビルド、テスト実行、開発サーバーの起動、データベースマイグレーションなどのすべての実行コマンドは、ホスト環境で直接実行するのではなく、原則として `docker compose exec` または `docker run` 等を介して Docker コンテナ内で実行してください。
   - これにより、ホスト側のシステム環境を汚染することなく、完全に隔離された安全なサンドボックス内で自律的にコード評価・実行が可能になります。

2. **コマンド承認の自動化設定（Allowed コマンド登録）:**
   - ホスト側の Antigravity CLI やエディタ連携におけるコマンド承認ポリシーにて、以下の「安全なコンテナ操作コマンドプレフィックス」を事前に `allowed` (承認不要) に登録することを人間に推奨します。
     - `docker compose exec`
     - `docker compose run`
     - `docker compose up`
     - `docker compose down`
